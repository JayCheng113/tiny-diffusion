...
class Block(nn.Module):
    ...
    def forward(self, x, attn_bias):
        x = x + self.attn(norm(x), attn_bias)
        x = x + self.mlp(norm(x))
        return x
...
class Model(nn.Module):
    def __init__(self):
        ...
        # ALiBi attention bias (symmetric distance form for bidirectional attention)
        self.alibi_seq_len = block_size
        self.register_buffer(
            "alibi_bias",
            self._build_alibi_bias(self.alibi_seq_len),
            persistent=False,
        )
    ...
    def _get_alibi_slopes(self, num_heads):
        ...
        lower_pow2 = 2 ** math.floor(math.log2(num_heads))
        slopes = slopes_power_of_two(lower_pow2).tolist()
        extra = slopes_power_of_two(2 * lower_pow2).tolist()[0::2]
        slopes.extend(extra[: num_heads - lower_pow2])
        return torch.tensor(slopes, dtype=torch.float32)

    def _build_alibi_bias(self, seq_len, device=None):
        if device is None:
            device = self.token_emb.weight.device
        slopes = self._get_alibi_slopes(n_head).to(device).view(1, n_head, 1, 1)
        t = torch.arange(seq_len, device=device)
        distance = (t[:, None] - t[None, :]).abs().to(torch.float32)
        distance = distance.view(1, 1, seq_len, seq_len)
    ...
    def forward(self, idx, targets=None, mask=None):
        ...
        assert T <= self.alibi_bias.size(-1)
        attn_bias = self.alibi_bias[:, :, :T, :T].to(dtype=x.dtype, device=x.device)
...
if __name__ == "__main__":
    ...
    weights_path = os.getenv("TD_WEIGHTS_PATH", "weights/diffusion_alibi.pt")
