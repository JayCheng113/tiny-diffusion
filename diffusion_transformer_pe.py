...
class Model(nn.Module):
    def __init__(self):
        ...
        pos_emb = self._precompute_sinusoidal_embeddings(block_size)
    ...
    def _precompute_sinusoidal_embeddings(self, seq_len, base=10000, device=None):
        if device is None:
            device = self.token_emb.weight.device
        channel_range = torch.arange(0, n_embd, 2, dtype=torch.float32, device=device)
        inv_freq = 1.0 / (base ** (channel_range / n_embd))
        t = torch.arange(seq_len, dtype=torch.float32, device=device)
        freqs = torch.outer(t, inv_freq)
        pos_emb = torch.zeros(seq_len, n_embd, dtype=torch.float32, device=device)
...
if __name__ == "__main__":
    ...
    weights_path = os.getenv("TD_WEIGHTS_PATH", "weights/diffusion_transformer_pe.pt")
