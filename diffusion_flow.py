...
class Model(nn.Module):
    ...
    def encode_hidden_from_embeddings(self, x):
        B, T, _ = x.size()
        x = norm(x)
    ...
    def encode_hidden(self, idx):
        ...
        return self.encode_hidden_from_embeddings(x)

    def forward_flow(self, x_t, t):
        # x_t: (B, T, n_embd), t: (B,)
        h = self.encode_hidden_from_embeddings(x_t)
...
def compute_flow_loss(model, targets, mask=None):
    # Build linear interpolation states x_t = (1-t) x1 + t x0 in embedding space.
    with torch.no_grad():
...
def generate(
    ...
    while len(all_tokens) - prompt_len < max_new_tokens:
        ...
        while masked.any():
            ...
            # Decode high-confidence masked positions with an adaptive threshold (or at least 1).
            adaptive_threshold = max(0.60, confidence_threshold - 0.25 * (1.0 - remaining_ratio))
...
if __name__ == "__main__":
    ...
    weights_path = "weights/diffusion_flow.pt"
